...
const { applyLive } = require('./livescores');
...
async function buildTournament({ fetcher, sourceUrl, liveEntries = null, now = Date.now() }) {
  const raw = await fetcher.get(sourceUrl, SOURCE_TTL);
  const rawMatches = (raw.matches || []).map((m, i) => ({ ...m, num: m.num || i + 1 }));
  if (liveEntries) applyLive(rawMatches, liveEntries); // 'post' scores commit before standings

  const matches = rawMatches.map((mm) => {
    const { _live, ...m } = mm;
    ...
    return {
      ...
      kickoff,
      status: _live ? 'live' : matchStatus(m, kickoff, now),
      live: _live ? { score: _live.score, clock: _live.clock } : null,
      goals1: m.goals1 || (_live && _live.goals1.length ? _live.goals1 : undefined),
      goals2: m.goals2 || (_live && _live.goals2.length ? _live.goals2 : undefined)
    };
  });
  ...
  const scenarios = allScenarios(groups, standings, matches, thirdPlace);
  ...
  return { generatedAt: new Date(now).toISOString(), name: raw.name || 'FIFA World Cup 2026', teams, groups, standings, thirdPlace, scenarios, matches, bracket };
}
